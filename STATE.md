...
- **2026-09-24**: `STATE.md` versionado otra vez, revisado sin rutas locales ni datos personales. Revierte el `git rm --cached` de 2026-08-28.
- **2026-08-28b**: Fix de wiki-links duplicados. Los corchetes se pintaban dos veces (`[[[[nota]]]]`): `links.js` los metía en el texto del `<a>` y el CSS del rediseño "Editor pro" los añadía otra vez con `::before`/`::after`. Detectado al preparar la captura del README. `links.js` deja ahora solo el nombre en el texto y los corchetes los dibuja el CSS, que era la intención del rediseño. El handler de doble clic reconstruye `[[nombre]]` desde `data-note` porque `innerText` no incluye los pseudo-elementos, así que `jumpToLine` sigue recibiendo lo mismo que antes. Verificado que nada más dependía de ese texto: `renderBacklinks` (que también los mete) no se importa en ningún sitio, y la barra de estado usa `findBacklinks` sobre el contenido crudo del archivo. Captura del README añadida en `docs/memoria-demo.png`, hecha contra una carpeta de notas de demo.
- **2026-08-28**: Repaso final y paso a `2_finished`. Carpeta renombrada de `MEMORIA markdown` a `memoria`, igual que el repo. `STATE.md` sacado del control de versiones (`git rm --cached`) y añadido a `.gitignore`: repo público terminado, el archivo lleva rutas locales y notas de sesión. README ampliado con los archivos sin extensión que `isTextFile()` ya abría y no estaban documentados (dotfiles y nombres conocidos tipo `Dockerfile`, `LICENSE`). Verificado contra el código: los 9 módulos, los 6 atajos, las extensiones de texto y multimedia y los CDN del README coinciden con lo implementado; demo de Pages responde HTTP 200. Los dos pendientes de 2026-06-16 (números de línea, scroll horizontal) pasan a ideas v2, no bloquean.
...
### Ideas v2 (no bloquean, cerradas 2026-08-28)
Ninguna de las dos impide dar el proyecto por terminado. Quedan aquí por si se retoma.
